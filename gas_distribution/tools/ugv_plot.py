...
# # simple ugv 1102
# EXPLORE_START_SEC = 1667382346
# EXPLORE_END_SEC = 1667382584
# SCAN_START_SEC = 1667382458

# cooperative 0118
EXPLORE_START_SEC = 1674027640
EXPLORE_END_SEC = 1674027852
SCAN_START_SEC = 1674027765
...
for topic, msg, t in bag.read_messages():
    if t.secs > EXPLORE_START_SEC and t.secs < EXPLORE_END_SEC:
        ...
        if topic=="/is_finish_search":
            status=np.array([0.0, 0.0, 0.0])
            status[1] = t.secs
            status[2] = t.nsecs
            if exchange_time_data is None:
                exchange_time_data=status

# exchange_time_data = exchange_time_data[0]
print(exchange_time_data)

max_index = np.argmax(scannning_data[:,0])
max_scannning_data = scannning_data[max_index, :]
print(max_scannning_data)
# print(max_tvoc_data)
...
def toFormatedTime(sec, nsec):
    return toRawTime(sec, nsec)- EXPLORE_START_SEC
...
fig1.plot(toFormatedTime(tvoc_data[:,1], tvoc_data[:,2]), tvoc_data[:,0], 'b')
# tmp: explore time has to be rechecked.
# fig1.axvline(peak_time, color='b', lineStyle='dotted', label="whole peak")
fig1.axvline(toFormatedTime(exchange_time_data[1], exchange_time_data[2]), color='k', lineStyle='dotted', label="start of scrutinizing")
fig1.axvline(toFormatedTime(max_scannning_data[1], max_scannning_data[2]), color='c', lineStyle='dotted', label="peak in scrutinizing")
...
# save
# plt.show()
fig.savefig('ugv_plot.eps', bbox_inches="tight", pad_inches=0.05)
fig.savefig('ugv_plot.pdf', bbox_inches="tight", pad_inches=0.05)

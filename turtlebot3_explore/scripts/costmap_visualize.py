...
# This script is temporary script for refine costmap in rviz.

class refine_gas_map:
    def __init__(self):

        rospy.init_node("refine_gas_map")
        self.gas_map_pub= rospy.Publisher("/refine_costmap", OccupancyGrid, queue_size=1)
        self.tvoc_sub = rospy.Subscriber("/tvoc", UInt16, self.gas_callback)
        ...
        self.gas_visual_map.info.resolution = rospy.get_param("~gas_visual_map_resolution", 0.2) # 0.05 m/pixel
        origin = rospy.get_param("~gas_visual_map_origin", [-5.0, -5.0, 0.0])
        print(origin)
    ...
    def odom_callback(self, msg):
        ...
        if self.gas_value != None:
            ...
            self.gas_visual_map.data[pixel_y * self.gas_visual_map.info.width + pixel_x] = self.gas_value
...
if __name__ == "__main__":
    try:
        refine_gas_map_action = refine_gas_map()
